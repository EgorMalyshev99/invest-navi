...
```txt
routes (TanStack Router) → widgets → features → entities → shared
```

В `apps/dashboard` верхний композиционный слой — `src/routes/`. Отдельный слой `views/pages` **не используется**.
...
4. **`src/routes/`:** routing, layout guards, page composition из widgets
5. `apps/landing` использует Server Components по умолчанию; dashboard — SPA client React
6. Если route разрастается — вытаскивать в **widget**, а не в отдельный слой
...
9. **shadcn/ui primitives** — `packages/ui` (импорт `@repo/ui/...`); не `src/components/`
...
Phase 9 ✅ — OAuth (Yandex ID, Google) + split frontend (`apps/landing`, `apps/dashboard`, `@repo/ui`); profile OAuth link/unlink; auth UX polish.
Phase 10 ✅ — контент лендинга, еженедельный обзор рынка, Vitest + Playwright, auth hardening, OpenAPI REST.
